...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        print(f"[AuthMiddleware] Cookies received: {list(all_cookies.keys())}")
        ...
        try:
            ...
            # Validate token - must not be empty and must be reasonable length
            if not token_to_use or len(token_to_use) > 500:
                return JSONResponse(
                    status_code=401,
                    content={"detail": "Invalid session token format"}
                )

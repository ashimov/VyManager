...
interface DeleteCommunityListRuleModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
  communityListName: string;
  rule: CommunityListRule | null;
}

export function DeleteCommunityListRuleModal({
  open,
  onOpenChange,
  onSuccess,
  communityListName,
  ...
  const handleDelete = async () => {
    ...
    try {
      await communityListService.deleteRule(communityListName, rule.rule_number);
      onOpenChange(false);
      onSuccess();
    } catch (err) {
